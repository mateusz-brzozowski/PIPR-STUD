...
class Plotter:
    ...
    def get_data(
        ...
        for column in columns.values():
            if column[0] == "winner_name":
                data = self.database.get_winratio_columns(
                    player1, player2, tourney_name, column, min_date, max_date)
                player1_desc, player1_rato = self.get_description(
                    data[0], data[1])
                player2_desc, player2_rato = self.get_description(
                    data[2], data[3])
                ...
                if best or column[0] == "winner_ht":
                    data = self.database.get_best_in_columns(
                        player1, player2, tourney_name,
                        column, min_date, max_date)
                else:
                    data = self.database.get_sum_in_columns(
                        player1, player2, tourney_name,
                        column, min_date, max_date)
                player1_desc, player1_rato = self.get_description(
                    data[0], data[1])
    ...
    def get_plot(
        ...
        ax[0] = self.round_bars(ax[0])
        ax[2] = self.round_bars(ax[2])
